...
通常のローカル集計は `start.bat`、Sites用の安全な集計・プレビューは `start_sites.bat` が入口です。README、`AGENTS.md`、ライセンス、プライバシー／セキュリティ文書、依存関係定義以外の実装ファイルはルートに置きません。`AGENTS.md`はエージェント向けの作業規約であり、利用者向けの起動入口ではありません。
