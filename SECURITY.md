...
このツールは ChatGPT 会話エクスポートという機密性の高いデータを扱います。通常の解析処理は外部へデータを送信しません。表示用 HTTP サーバーは `127.0.0.1` にだけバインドします。

ただし、入力はローカルで実行される Python によって解析されます。悪意がある、破損している、または信頼できないエクスポートを入力しないでください。

生のエクスポートと、生成された HTML / JSON / CSV を Git に追加してはいけません。生成物にはタイトルなど会話由来の情報が残ることがあります。生成 HTML を第三者に公開・送付する前に、必ず内容を確認してください。
