...
- ブラウザ: `fetch` と ES2017 を実装したブラウザが必要です。Playwrightのヘッド付きChromium（UA: Chrome/150.0.0.0）で、1280px幅と390px幅の表示、月切り替え、JSON取得、画面間遷移を検証済みです。他のブラウザは未検証です。
- Git は必須ではありません。GitHub の **Download ZIP** を展開しても利用できます。
- インターネット接続は、初回の `pip install -r requirements.txt`、`tiktoken` が初めて語彙データを取得する場合、GitHub Pages デモの閲覧時に必要になることがあります。会話エクスポート自体は外部送信しません。語彙取得に失敗しても、解析は文字数ベース推定へフォールバックします。
